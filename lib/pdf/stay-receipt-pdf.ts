...
function receiptFontPaths() {
  const fontDirectory = join(process.cwd(), "public", "fonts");
  const files = {
    regular: join(fontDirectory, "NotoSansSC-Regular.otf"),
    bold: join(fontDirectory, "NotoSansSC-Bold.otf"),
  };
  for (const fontPath of Object.values(files)) {
    if (!existsSync(fontPath)) throw new Error(`PDF font is required but unavailable: ${fontPath}`);
  }
  return files;
}
const labels = {
...
export async function generateStayReceiptPdf(data: StayReceiptData, options: StayReceiptOptions = {}) {
  ...
  doc.registerFont("NotoSansSC", files.regular);
  doc.registerFont("NotoSansSC-Bold", files.bold);
  doc.font("NotoSansSC");
  ...
  doc.font("NotoSansSC-Bold").fontSize(22).fillColor(ink).text(text.title, left, 42, { width, align: "center" }); doc.moveTo(left, 78).lineTo(right, 78).lineWidth(1.2).strokeColor(accent).stroke();
  doc.font("NotoSansSC-Bold").fontSize(13).fillColor(ink).text(`${data.customerName}${text.honorific}`, left, 101, { width: 280 });
  doc.moveTo(left, 123).lineTo(320, 123).lineWidth(0.5).strokeColor(line).stroke();
  const metaX = 354;
  [[text.receiptDate, dateLabel(data.receiptDate, locale)], [text.receiptNo, data.receiptNumber], [text.bookingNo, data.bookingNumber]].forEach(([label, value], index) => { const y = 96 + index * 20; doc.font("NotoSansSC").fontSize(8).fillColor(muted).text(label, metaX, y, { width: 70 }); doc.font("NotoSansSC-Bold").fontSize(8.5).fillColor(ink).text(value, metaX + 70, y, { width: 129, align: "right" }); });
  ...
  doc.font("NotoSansSC-Bold").fontSize(9).fillColor(ink).text(issuerName, 342, 217, { width: 211, align: "right" }); doc.font("NotoSansSC").fontSize(7.5).fillColor(muted).text(issuerAddress, 322, 233, { width: 231, align: "right" }); doc.font("NotoSansSC").text(text.registrationNo, 342, 247, { width: 105, align: "right" }); doc.font("NotoSansSC").text(registrationNumber, 450, 247, { width: 103, align: "right" });
  doc.font("NotoSansSC").fontSize(9).fillColor(ink).text(text.statement, left, 171); doc.roundedRect(left, 193, 270, 68, 3).fill(soft).strokeColor(line).stroke(); doc.font("NotoSansSC").fontSize(8).fillColor(muted).text(text.total, left + 14, 207); doc.font("NotoSansSC-Bold").fontSize(22).fillColor(ink).text(money(data.totalAmount, locale), left + 14, 226, { width: 242, align: "right" });
  let y = 285; const columns = [left, 334, 397, 470, right]; doc.rect(left, y, width, 27).fill(accent);
  [[text.description, columns[0], columns[1] - columns[0], "left"], [text.quantity, columns[1], columns[2] - columns[1], "center"], [text.unitPrice, columns[2], columns[3] - columns[2], "right"], [text.amount, columns[3], columns[4] - columns[3], "right"]].forEach(([label, x, cellWidth, align]) => doc.font("NotoSansSC-Bold").fontSize(8).fillColor("#FFFFFF").text(String(label), Number(x) + 7, y + 8, { width: Number(cellWidth) - 14, align: align as "left" | "center" | "right" })); y += 27;
  data.items.forEach((item, index) => { const rowHeight = 28; if (index % 2 === 1) doc.rect(left, y, width, rowHeight).fill(soft); const description = item.kind === "accommodation" ? text.accommodation(item.nights ?? 0) : text[item.kind]; doc.font("NotoSansSC").fontSize(8).fillColor(ink).text(description, columns[0] + 7, y + 9, { width: columns[1] - columns[0] - 14, ellipsis: true, lineBreak: false }); doc.font("NotoSansSC").text(String(item.quantity), columns[1] + 7, y + 9, { width: columns[2] - columns[1] - 14, align: "center" }); doc.text(money(item.unitPrice, locale), columns[2] + 7, y + 9, { width: columns[3] - columns[2] - 14, align: "right" }); doc.font("NotoSansSC-Bold").text(money(item.amount, locale), columns[3] + 7, y + 9, { width: columns[4] - columns[3] - 14, align: "right" }); doc.moveTo(left, y + rowHeight).lineTo(right, y + rowHeight).lineWidth(0.4).strokeColor(line).stroke(); y += rowHeight; });
  while (y < 480) { doc.moveTo(left, y + 28).lineTo(right, y + 28).lineWidth(0.25).strokeColor("#CBD5E1").stroke(); y += 28; }
  const tax = Math.floor(data.totalAmount * 10 / 110), taxBase = data.totalAmount - tax, summaryX = 334; doc.rect(summaryX, 511, right - summaryX, 56).strokeColor(line).stroke(); doc.font("NotoSansSC").fontSize(8).fillColor(ink).text(text.breakdown, summaryX + 8, 520); doc.text(text.taxable, summaryX + 49, 520, { width: 92 }); doc.font("NotoSansSC-Bold").text(money(taxBase, locale), summaryX + 142, 520, { width: 68, align: "right" }); doc.font("NotoSansSC").text(text.tax, summaryX + 49, 542, { width: 92 }); doc.font("NotoSansSC-Bold").text(money(tax, locale), summaryX + 142, 542, { width: 68, align: "right" });
  ...
  doc.rect(left, 585, width, 108).strokeColor(line).stroke(); doc.font("NotoSansSC-Bold").fontSize(8.5).fillColor(ink).text(text.notes, left + 10, 596); doc.font("NotoSansSC").fontSize(8).fillColor(muted).text(text.stayCharge, left + 10, 618, { width: 92 }); doc.font("NotoSansSC").text(data.listingName, left + 105, 618); doc.font("NotoSansSC").text(text.stayPeriod, left + 10, 636, { width: 92 }); doc.font("NotoSansSC").text(`${data.checkInDate} - ${data.checkOutDate}`, left + 105, 636); doc.font("NotoSansSC").text(text.paymentMethod, left + 10, 654, { width: 92 }); doc.font("NotoSansSC").text(paymentLabel, left + 105, 654); doc.font("NotoSansSC").fontSize(7).fillColor(muted).text("1 / 1", left, 784, { width, align: "center", lineBreak: false });
