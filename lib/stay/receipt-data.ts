...
export async function getStayReceiptData(id: string, customerId?: string): Promise<StayReceiptData | null> {
  let query = createSupabaseAdminClient().from("stay_bookings").select("booking_number,customer_id,guest_name,check_in_date,check_out_date,nights,subtotal,additional_guest_fee,cleaning_fee,discount_amount,total_amount,card_fee_amount,payment_method,payment_status,paid_at,stay_listings(name)").eq("id", id);
  if (customerId) query = query.eq("customer_id", customerId);
  const { data: booking, error } = await query.maybeSingle();
  if (error || !booking || booking.payment_status !== "paid" || !booking.paid_at) return null;
  ...
  return {
    receiptNumber: `${booking.booking_number}-R1`, receiptDate: dateInJapan(booking.paid_at), customerName: booking.guest_name,
