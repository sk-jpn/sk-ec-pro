...
export async function POST(request: Request) {
  ...
  if (session.metadata?.paymentType === "stay") {
    ...
    const { data: booking, error: lookupError } = await supabase.from("stay_bookings").select("id,total_amount,card_fee_amount,payment_status,stripe_checkout_session_id").eq("id", bookingId).eq("booking_number", bookingNumber).maybeSingle();
    if (lookupError || !booking || booking.total_amount + booking.card_fee_amount !== expectedAmount || booking.stripe_checkout_session_id !== session.id) return Response.json({ message: "宿泊予約が一致しません。" }, { status: 400 });
