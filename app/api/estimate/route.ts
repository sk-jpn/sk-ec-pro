...
function adminEmail(data: EstimateRequest, receivedAt: string) {
  ...
  try {
    ...
    for (let productIndex = 0; productIndex < validatedFiles.length; productIndex += 1) {
      for (let imageIndex = 0; imageIndex < validatedFiles[productIndex].length; imageIndex += 1) {
        ...
        const { error: recordError } = await admin.from("estimate_item_images").insert({ estimate_item_id: items[productIndex].id, storage_path: path, original_name: file.originalName, mime_type: file.mimeType, sort_order: imageIndex + 1 });
    ...
    const { error } = await resend.batch.send([
      {
        ...
        html: adminEmail(data, receivedAt),
      },
      {
        from,
