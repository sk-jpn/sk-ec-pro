...
export default async function NewRidePage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const stays=(bookings??[]).map(row=>{const listing=row.stay_listings as unknown as {code:string;name:string}|null;return{id:row.id,label:`${listing?.code??listing?.name??"宿泊"}（${row.check_in_date}〜${row.check_out_date}）`}});
