...
export async function startRideStripeCheckout(formData:FormData){
  ...
  try{
    ...
    if(!alreadyPaid){
    const fee=Math.round(ride.total_amount*Number(ride.card_fee_rate??0)/100),expected=ride.total_amount+fee,origin=new URL(process.env.SITE_URL||"https://formosajapan.com").origin,email=(ride.stay_customers as unknown as {email:string}|null)?.email;
