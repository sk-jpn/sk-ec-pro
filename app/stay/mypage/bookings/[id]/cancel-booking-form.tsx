...
export function CancelBookingForm({ id, compact = false }: { id: string; compact?: boolean }) {
  ...
  if (!open) return <StayLocalized><Button type="button" variant="destructive" size={compact ? "sm" : "default"} onClick={() => setOpen(true)}>キャンセル</Button></StayLocalized>;
  return <StayLocalized><form action={customerBookingAction} className="grid min-w-0 w-full gap-2 sm:grid-cols-[minmax(0,1fr)_auto_auto]">
