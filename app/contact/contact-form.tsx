...
export function ContactForm() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch(withBasePath("/api/contact"), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(Object.fromEntries(form)) });
  ...
  return <form onSubmit={submit} className="rounded-[2rem] border border-slate-100 bg-white p-5 shadow-[0_20px_60px_-45px_rgba(15,23,42,.35)] sm:p-8">
    <div className="grid gap-6 sm:grid-cols-2"><label className="text-sm font-medium text-slate-700">お名前 <span className="text-red-600">*</span><input required name="name" autoComplete="name" className={field} /></label><label className="text-sm font-medium text-slate-700">メールアドレス <span className="text-red-600">*</span><input required type="email" name="email" autoComplete="email" className={field} /></label><label className="text-sm font-medium text-slate-700 sm:col-span-2">お問い合わせ種別<select name="topic" className={field}><option>サービスについて</option><option>当社オリジナル製品について</option><option>当社運営ショップについて</option><option>その他</option></select></label><label className="text-sm font-medium text-slate-700 sm:col-span-2">お問い合わせ内容 <span className="text-red-600">*</span><textarea required name="message" rows={7} className={`${field} py-3`} /></label></div>
    <div className="absolute -left-[10000px] top-auto size-px overflow-hidden" aria-hidden="true"><label>ウェブサイト<input name="website" type="text" tabIndex={-1} autoComplete="off" /></label></div>
    <p className="mt-5 text-xs leading-6 text-slate-500">見積をご希望の場合は、商品画像を添付できる無料見積フォームをご利用ください。</p>
    <TurnstileWidget ref={turnstileRef} action="contact" onTokenChange={handleTokenChange} />
    {error && <p role="alert" className="mt-4 rounded-xl bg-red-50 p-4 text-sm font-semibold text-red-700">{error}</p>}
    <button disabled={sending || !turnstileToken} className="mt-7 inline-flex min-h-13 w-full items-center justify-center gap-2 rounded-full bg-blue-600 px-7 text-sm font-semibold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60">{sending ? "送信中…" : "お問い合わせを送信"}<Send size={17} /></button>
  </form>;
}
