...
export function StatusBadge({ status }: { status: EstimateStatus }) { const variant = status === "新規" ? "danger" : status === "見積作成中" ? "warning" : ["お客様確認中", "中国発送", "国際配送中", "国内発送"].includes(status) ? "info" : ["approved", "paid", "発注済", "完了"].includes(status) ? "success" : status === "キャンセル" ? "secondary" : "outline"; return <Badge variant={variant}>{estimateStatusLabel(status)}</Badge>; }
