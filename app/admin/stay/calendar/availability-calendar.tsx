...
type CalendarDay = null | { day: number; date: string; available: boolean; selectable: boolean; past: boolean; listingClosed: boolean; bookings: { id: string; number: string; status: string }[]; blocks: { id: string; label: string; reason: string; isAirbnb: boolean }[] };
...
export function AvailabilityCalendar({ days, today, listingId, maxGuests, customers, initialStart, initialLastNight }: { days: CalendarDay[]; today: string; listingId: string; maxGuests: number; customers: Customer[]; initialStart: string; initialLastNight: string }) {
  ...
  const selectableDates = useMemo(() => new Set(days.filter((day) => day?.selectable).map((day) => day!.date)), [days]);
  const daysByDate = useMemo(() => new Map(days.filter((day): day is NonNullable<CalendarDay> => Boolean(day)).map((day) => [day.date, day])), [days]);
  const selectedDates = useMemo(() => { const result = new Set<string>(); if (!start) return result; let cursor = start; const end = lastNight || start; let count = 0; while (cursor <= end && count < 732) { result.add(cursor); cursor = addDay(cursor); count += 1; } return result; }, [start, lastNight]);

  function updateSelection(nextStart: string, nextLastNight: string) {
    setStart(nextStart); setLastNight(nextLastNight);
    const params = new URLSearchParams(searchParams.toString());
    if (nextStart) params.set("start", nextStart); else params.delete("start");
    if (nextLastNight) params.set("lastNight", nextLastNight); else params.delete("lastNight");
    router.replace(`?${params.toString()}`, { scroll: false });
  }

  function selectDate(date: string) {
    if (!selectableDates.has(date)) return;
    if (!start || lastNight || date < start) { updateSelection(date, ""); return; }
    let cursor = start;
    while (cursor <= date) { const day = daysByDate.get(cursor); if (day && !day.selectable) return; cursor = addDay(cursor); }
    updateSelection(start, date);
  }
  ...
  return <>
    <div className="grid grid-cols-7 overflow-hidden rounded-xl border border-slate-200">
      ...
      {days.map((entry, index) => entry ? <button type="button" key={entry.date} onClick={() => selectDate(entry.date)} disabled={!entry.selectable} className={`min-h-28 border-b border-r border-slate-100 p-2 text-left sm:min-h-36 sm:p-3 ${selectedDates.has(entry.date) ? "bg-blue-100 ring-2 ring-inset ring-blue-500" : entry.available ? "bg-emerald-50/40 hover:bg-emerald-100/70" : entry.selectable ? "bg-red-50/40 hover:bg-red-100/60" : entry.past ? "bg-slate-50 text-slate-400" : "bg-red-50/40"}`}>
    ...
    <div className="mt-5 rounded-xl border border-blue-200 bg-blue-50/50 p-4 sm:p-5"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h2 className="font-bold">オフライン予約を作成</h2><p className="mt-1 text-sm text-slate-600">管理者が選択可能な日を1日、または開始日と最終宿泊日の順に選択してください。Airbnbブロック日と過去日も選択できます。</p></div>{start && <Button type="button" variant="outline" onClick={() => updateSelection("", "")}>選択解除</Button>}</div>
    ...
    <div className="mt-4 flex flex-wrap gap-4 text-xs font-medium text-slate-600"><span className="flex items-center gap-2"><b className="grid size-6 place-items-center rounded-full bg-emerald-100 text-emerald-700">○</b>Airbnb開放日・選択可能</span><span className="flex items-center gap-2"><b className="grid size-6 place-items-center rounded-full bg-red-100 text-red-600">×</b>予約・ブロックあり</span><span className="rounded bg-blue-100 px-2 py-1 text-blue-700">Airbnbブロック日は×表示でも選択可能</span><span>過去の日付も選択可能</span><span>チェックアウト日は宿泊日に含めません。</span></div>
  </>;
}
