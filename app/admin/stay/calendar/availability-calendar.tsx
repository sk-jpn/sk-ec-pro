...
export function AvailabilityCalendar({ days, today, listingId, maxGuests, customers }: { days: CalendarDay[]; today: string; listingId: string; maxGuests: number; customers: Customer[] }) {
  ...
  function selectDate(date: string) {
    ...
    let cursor = start;
    while (cursor <= date) { if (!selectableDates.has(cursor)) return; cursor = addDay(cursor); }
    setLastNight(date);
  }
  ...
  return <>
    ...
    <div className="mt-5 rounded-xl border border-blue-200 bg-blue-50/50 p-4 sm:p-5"><div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center"><div><h2 className="font-bold">オフライン予約を作成</h2><p className="mt-1 text-sm text-slate-600">管理者が選択可能な日を1日、または開始日と最終宿泊日の順に選択してください。Airbnbブロック日と過去日も選択できます。</p></div>{start && <Button type="button" variant="outline" onClick={() => { setStart(""); setLastNight(""); }}>選択解除</Button>}</div>
