...
export default async function AccountEstimateDetailPage({ params }: PageProps<"/account/estimates/[id]">) {
  ...
  const { data, error } = await supabase.from("estimates").select("id, estimate_no, status, approved_at, paid_at, payment_method, china_shipping_fee, international_shipping_fee, agency_fee, other_fee, discount, tax, estimate_items(id, product_name, url, quantity, unit_price, color, size, model, request, estimate_item_images(id, storage_path, original_name, sort_order), received_item_images(id, storage_path, original_name, sort_order)), customers!inner(auth_user_id)").eq("id", id).eq("customers.auth_user_id", user.id).maybeSingle();
