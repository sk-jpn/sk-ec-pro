...
export default async function AccountLayout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-950">
    <header className="border-b border-slate-200 bg-white"><div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-5 py-4 sm:px-8"><Link href="/account"><Image src={withBasePath("/brand/sk-ec-pro-logo.png")} alt="SK EC Pro" width={160} height={60} className="h-auto w-32" /></Link><div className="flex min-w-0 items-center gap-2"><span className="hidden max-w-52 truncate text-sm text-slate-500 sm:block">{profile?.full_name || user.email}</span><form action={logoutCustomer}><Button variant="ghost" size="sm"><LogOut size={15} />ログアウト</Button></form></div></div></header>
